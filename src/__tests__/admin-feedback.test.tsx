...
interface MockOptions {
  getAdminFeedbackList: boolean;
}

const getAdminFeedbackList = (): Record<string, any>[] => {
  return [
    {
...
const buildMocks = (): {
  ...
  const _adminFeedbackList = getAdminFeedbackList();
  _mocks.push({
    request: {
...
describe("Admin feedback page",() => {
  ...
  test("Click view icon should open view feedback popup", async () => {
    await sut();
    fireEvent.click(screen.queryByTestId("viewIcon_12274a23-4932-49b6-9eec-ae7f9f6b804d"));
    // Check that the dialog is open.
    expect(screen.queryByTestId("bootstrapDialogue")).toBeTruthy();
  });
  ...
  test("Click Edit icon should open Edit feedback popup", async () => {
    await sut();
    fireEvent.click(screen.queryByTestId("editIcon_12274a23-4932-49b6-9eec-ae7f9f6b804d"));
    // Check that the dialog is open.
    expect(screen.queryByTestId("bootstrapDialogue")).toBeTruthy();
  });
  ...
  test("Click Delete icon should open Delete feedback popup", async () => {
    await sut();
    fireEvent.click(screen.queryByTestId("deleteIcon_12274a23-4932-49b6-9eec-ae7f9f6b804d"));
    // Check that the dialog is open.
    expect(screen.queryByTestId("bootstrapDialogue")).toBeTruthy();
  });
