...
export const allAnsColSum = (templateData) => {
    const { questions: { bodyRows, footerRows } } = templateData;
    let total = {
        col2: 0,
        col3: 0,
        col4: 0,
    ...
    bodyRows.forEach((item) => {
        const { answer } = item
        total[answer] += !isNaN(parseInt(item[answer])) ? parseInt(item[answer]) : 0;
        overAllTotal["colAvg"] += !isNaN(parseInt(item[answer])) ? parseInt(item[answer]) : 0
    })
    return { tableFooter: [{ ...footerRows[0], ...total }, { ...footerRows[1], ...overAllTotal }], overAllTotal }
}

export const onAddQuesionBox = ({ templateData, setFieldValue, confirmModalRef }) => {
