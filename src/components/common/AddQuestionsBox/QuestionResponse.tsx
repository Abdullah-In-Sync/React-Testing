...
const QuestionResponse: React.FC<ViewProps> =  ({
     i, item, formikProps, isEditView
}) => {

    console.log({item})
    const { setFieldValue } = formikProps

    const {  patientAnswer: answer, questionType, questionOption: answerOptions   
    } = item
            ...
            <RadioGroup {...row} className="radio-buttons" defaultValue={answer} >
                {(options as Array<any>).map((option: string, index: number) => (
                    <FormControlLabel
                        ...
                        control={<Radio />}
                        label={option}
                        onClick={()=>setFieldValue(`questions.${i}.patientAnswer`, option)}
                    />
                ))}
            </RadioGroup>
        );
            ...
            <Box className="textAnswerWrapper">
                <Box>
                    ...
                    <ResponseTextArea
                        ...
                        placeholder={(isEditView !== i+i)?"Please type response here":"No response"}
                        multiline
                    />
                </Box>
            </Box>
    ...
    return <Stack className="questionResponseSection">
        {    answers()}
    </Stack>
