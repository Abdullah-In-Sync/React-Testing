...
import CreateRelapsePlan from "../../../../pages/admin/relapsePlan/create";
...
mocksData.push({
  request: {
    query: ADMIN_CREATE_RELAPSE_PLAN,
    variables: {
  ...
  result: {
    data: {
      adminCreateRelapsePlan: {
        duplicateNames: [
          {
            _id: "4c784f6c-b5f4-4539-88b4-64a3ee044c18",
            name: "admin org",
...
const fillUpperForm = async () => {
  ...
  const planDescriptionInput = await screen.findByTestId("planDesc");
...
const submitDuplicatedFullForm = async () => {
  await sut();
...
describe("Admin relapse plan list", () => {
  ...
  it("show Error popup on if plan is already exit", async () => {
    await submitDuplicatedFullForm();
    const confirmButton = await screen.findByRole("button", {
      name: "Confirm",
    });
    fireEvent.click(confirmButton);
    const okButton = await screen.findByTestId("confirmButton");
    expect(okButton).toBeInTheDocument();
  });
